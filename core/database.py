...
class SystemUpdateRow(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    update_type = Column(String)   # e.g. "filter_threshold", "confidence_bump"
...
def update_trade_outcome(
    ...
    with get_session() as session:
        ...
        if row:
            ...
            row.settled_at = settled_at or datetime.utcnow()
            session.commit()
...
def get_trade_stats() -> dict:
    with get_session() as session:
        ...
        total_pnl = sum(r.pnl_usdc for r in pnl_rows)
